...
async fn fetch_manifest_signature() -> Result<Vec<u8>> {
    let sig_url = format!("{}.sig", manifest_url());
    let sig_hex = reqwest::get(&sig_url)
        .await
...
pub async fn check_and_update() -> Result<bool> {
    ...
    if let Err(e) = verify_manifest_signature(&manifest_bytes, &fetch_manifest_signature().await?) {
